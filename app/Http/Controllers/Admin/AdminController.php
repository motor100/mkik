...
class AdminController extends Controller
{
    ...
    public function news_add(Request $request)
    {
        ...
        $gallery = $request->file('inputgallery');
        ...
        $have_slug = DB::table('mainnews')
                        ->where('slug', $slug)
                        ->get();

        if (count($have_slug) > 0) {
            $newslug = $slug . '-%';
            $slugs = DB::table('mainnews')
        ...
        DB::insert('insert into mainnews (title, image, gallery, text, slug, year, date, excerpt, cat, alb, holdit, holddate, topn, created_at, updated_at) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [$title, $img, $glr, $text, $slug, $year, $date, $excerpt, $cat, $alb, $holdit, $holddate, $topn, $now, $now]);
    ...
    {   
        ...
        $napravlenie = DB::table('abiturientu_napravleniya_podgotovkis')
                            ->where('id', $id)
                            ->first();
        ...
        if ($gallery) {
            ...
            $glr = $napravlenie->gallery;
        }

        DB::table('abiturientu_napravleniya_podgotovkis')
            ->where('title', $direction)
            ->update([
                'chairman' => $chairman,
                'gallery' => $glr,
                'teachers' => $teachers,
        ...
        return redirect('/dashboard/abiturientu-napravleniya-podgotovki');
    }
